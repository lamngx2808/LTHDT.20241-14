...
public class Graph {
	...
	public void removeNode(Node n) {
		...
		connections.removeIf(connection -> connection.getNode1().equals(n) || connection.getNode2().equals(n));
	}
		...
		while (!queue.isEmpty()) {
			...
			for (Node neighbor : getNeighbors(current)) {
				double weight = connections.stream()
						...
						.findFirst().map(Connection::getLatency) // Sử dụng độ trễ làm trọng số
